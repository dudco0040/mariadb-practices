...
-- 예) 2014년 07월 10일
select date_format(min(hire_date), "%Y년 %m월 %d일")  -- 현재 근무중이어야 하지 않나? join 해야함 
from employees;
...
select date_format(now(), "%Y") - date_format(min(birth_date), "%Y"), date_format(now(), "%Y") - date_format(max(birth_date),"%Y")
from employees;